...
SQLAlchemy 2.0 with professional relational design
"""
import uuid
...
class Department(TimestampMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        ...
        Index("ix_departments_active", "is_active"),
    )
...
class User(TimestampMixin, SoftDeleteMixin, Base):
    ...
    approval_limit: Mapped[Decimal] = mapped_column(
    ...
    purchase_orders: Mapped[List["PurchaseOrder"]] = relationship(
        "PurchaseOrder", back_populates="requester", foreign_keys="PurchaseOrder.requester_id"
    )
...
class Vendor(TimestampMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        Index("ix_vendors_code", "vendor_code"),
...
class PurchaseOrder(TimestampMixin, SoftDeleteMixin, Base):
    ...
    __table_args__ = (
        ...
        Index("ix_po_requester", "requester_id"),
        Index("ix_po_vendor", "vendor_id"),
        Index("ix_po_number", "po_number"),
        Index("ix_po_created", "created_at"),
...
class ApprovalWorkflow(TimestampMixin, Base):
    ...
    steps: Mapped[List["ApprovalWorkflowStep"]] = relationship(
        "ApprovalWorkflowStep", back_populates="workflow", order_by="ApprovalWorkflowStep.order"
    )
...
class ApprovalWorkflowStep(TimestampMixin, Base):
    ...
    escalation_user_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        UUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )
...
class ApprovalStep(TimestampMixin, Base):
    ...
    __table_args__ = (
        Index("ix_approval_po", "purchase_order_id"),
...
class Budget(TimestampMixin, Base):
    ...
    __table_args__ = (
        Index("ix_budget_department", "department_id"),
        Index("ix_budget_year", "fiscal_year"),
    )
